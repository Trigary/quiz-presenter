...
<template>
  ...
  <div>
    <v-sheet v-for='ans in shuffledAnswers'
             class='ma-2 pa-2' rounded='pill'
             :color='revealed ? ans.style.value.revealed.color : ans.style.value.hidden.color'
             :style='"border: 2px solid " + (revealed ? ans.style.value.revealed.border : ans.style.value.hidden.border)'
  ...
  <ControlButton v-if='props.verdict !== null && revealed' activation_key='ArrowDown' @click='advance()'>
    DOWN Continue
  </ControlButton>
</template>
